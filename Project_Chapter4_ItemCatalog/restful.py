...
from flask import Flask, render_template, request, redirect, jsonify, url_for, abort, g, Blueprint, make_response
...
def login(provider):
    ...
    # FACEBOOK REST API
        ...
        url = 'https://graph.facebook.com/oauth/access_token?grant_type=fb_exchange_token&client_id=%s&client_secret=%s&fb_exchange_token=%s' %(app_id, app_secret, auth_code)  #access_token
        h = httplib2.Http()
    ...
    if not user:
        user = User(user_name = name, user_picture = picture, user_email = email)
        session.add(user)
        session.commit()
    # Make token
